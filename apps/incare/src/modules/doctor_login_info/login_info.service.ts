...
import { DoctorLoginInfoDocument, DotorLoginInfo } from './login_info.schema'

@Injectable()
export class DoctorLoginInfoService {
  private response: WaffleResponse
  constructor(
    @InjectModel(DotorLoginInfo.name, 'sharedConnection')
    private readonly loginInfoModel: Model<DoctorLoginInfoDocument>,
  ) { }
  ...
  async create(createLoginInfoDto: Partial<DotorLoginInfo>): Promise<DotorLoginInfo> {
  ...
  async getAllUserLoginInfo(user: any) {
    if (user && Roles.Admin === user.role) {
      const loginInfos: DoctorLoginInfoDocument[] = await this.loginInfoModel.find(
        {},
      )
  ...
  async findUserToken(
    user_id: string,
    jti: string,
  ): Promise<DoctorLoginInfoDocument> {
    return await this.loginInfoModel.findOne({ user_id, jti, is_active: true })
  }

  async findUserTokenById(_id: string): Promise<DoctorLoginInfoDocument> {
